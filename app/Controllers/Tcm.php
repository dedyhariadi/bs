...
class Tcm extends BaseController
{
    ...
    private function getSatkaiName($satkaiId)
    {
        $satkai = $this->satkaiModel->find($satkaiId);
        return $satkai ? $satkai['satkai'] : 'Unknown';
    }

    public function trxtcm($idKegiatan)
    {
        if ($this->request->getVar('tcmId')) {
            $currentKegiatan = $this->kegiatanModel->find($idKegiatan);

            // Insert transaksi baru yang menunjukkan perpindahan dari transferDari ke transferKe
            $dataTrx = [
            ...
            session()->setFlashdata('success', 'Berhasil memindahkan TCM dari ' . $this->getSatkaiName($currentKegiatan['transferDariId']) . ' ke ' . $this->getSatkaiName($currentKegiatan['transferKeId']));
        }
        ...
        $data = [
            ...
            'tcmByTransferDariId' => $this->trxTcmModel->getAvailableTcmByTransferDariId($currentKegiatan['transferDariId'], $idKegiatan),
            'trxTcmbyKegiatan' => $this->trxTcmModel->getTrxTcmByKegiatanId($idKegiatan),
            'kegiatan' => $currentKegiatan, // Fetch all kegiatan records if needed
        ];
    ...
    public function historyPenempatan($idTcm)
    {
        $tcm = $this->tcmModel->find($idTcm);
        $historyPenempatan = $this->trxTcmModel->getHistoryPenempatan($idTcm);

        $data = [
            'title' => 'History Penempatan TCM',
            'tcm' => $tcm,
            'historyPenempatan' => $historyPenempatan,
        ];

        return view('tcm/historyPenempatan', $data);
    }
}
